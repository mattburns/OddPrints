...
public class Coupons {
    ...
    public Viewable get() throws URISyntaxException {
        ...
        it.put("noCodeGiven", true);
        return new Viewable("/coupons", it);
    }

    @GET
    @Produces(MediaType.TEXT_HTML)
    @Path("/{code}")
    public Viewable getCode(@Context HttpServletRequest req,
            @PathParam("code") String code) throws URISyntaxException {
        PersistenceManager pm = PMF.get().getPersistenceManager();
        Map<String, Object> it = Maps.newHashMap();

        Coupon coupon = Coupon.getCouponByCode(code, pm);
        if (coupon != null) {
