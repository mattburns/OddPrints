...
import com.oddprints.CheckoutNotificationHandler;
...
public class CheckoutNotification {
    ...
    @POST
    @Path("/{environment}")
    public Response doPost(@PathParam("environment") String environment,
            @Context HttpServletRequest request,
            @Context HttpServletResponse response) {
        ApiContext apiContext = Environment.valueOf(environment.toUpperCase())
                .getCheckoutAPIContext();

        apiContext.handleNotification(new CheckoutNotificationHandler(request,
                response));
